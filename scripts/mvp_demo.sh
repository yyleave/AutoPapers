...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    -h|--help)
      cat <<'EOF'
Usage: scripts/mvp_demo.sh [--mode offline|hybrid] [--workdir /tmp/path]
...
echo
echo "[1/7] Create tiny local PDF fixture"
uv run python - <<'PY'
...
echo
echo "[2/7] Init profile and patch keywords to local PDF dir"
...
echo
echo "[3/7] Phase1 run (search + fetch-first + parse-fetched)"
...
echo
echo "[4/7] Build and inspect corpus snapshot"
...
echo
echo "[5/7] Draft/confirm/export proposal"
